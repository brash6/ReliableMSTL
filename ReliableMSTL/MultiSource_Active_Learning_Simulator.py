...
def MultiSource_Active_Learning_Simulator(exp_params):
    ...
    for j in range(30):
        ...
        if exp_params["generator"] == "gaussian":
            source_labeled_indices, X_source, Y_source, X_target_train, X_target_test, Y_target_test = gaussian_generator(
                exp_params)
        elif exp_params["generator"] == "uniform":
            source_labeled_indices, X_source, Y_source, X_target_train, X_target_test, Y_target_test = uniform_generator(
                exp_params)
        ...
        for i in range(X_target_test.shape[0]):
            Y_predicted_before_active_learning.append(
                model.multi_source_classify_PWMSTL(X_target_test[i].reshape(1, -1)))
        ...
        for i in range(X_target_test.shape[0]):
            Y_predicted_after_active_learning.append(
                model.multi_source_classify_PWMSTL(X_target_test[i].reshape(1, -1)))
