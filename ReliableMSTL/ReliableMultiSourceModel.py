...
class ReliableMultiSourceModel(object):
    ...
    def initialize_tau(self):
        ...
        if self.start_mode == "cold":
            ...
            for source_i in range(self.n_domain):
    ...
    def initialize_unlabeled_indices(self):
        ...
        """
        for source_id in range(self.n_domain):
    ...
    def reset_unlabeled_indices_for_source(self, source_id):
        ...
        """
        self.source_unlabeled_indices[source_id] = [i for i in range(self.X_source[source_id].shape[0]) if
    ...
    def train_source_constant_mmd_weights(self):
        ...
        self.source_constant_prox_weights = self.source_constant_prox_weights / np.sum(
            self.source_constant_prox_weights)
    ...
    def perform_active_learning(self, source_i=-1):
        ...
        if self.AL_method == "random":
            ...
            if source_i == -1:
                source_i = random.choice(range(self.n_domain))
    ...
    def query_instance(self, source_i, active_index, retrain=False):
        ...
        self.Y_source_labeled[source_i] = np.append(self.Y_source_labeled[source_i],
                                                    self.Y_source[source_i][active_index][0])
        self.source_unlabeled_indices[source_i].remove(active_index)
        if retrain:
            X_train, Y_train = self.X_source[source_i][self.source_labeled_indices[source_i]], self.Y_source_labeled[
                source_i]
    ...
    def pick_active_index(self, source_id, criteria):
        ...
        if criteria == "random":
            ...
            uncertainty_list = np.abs(self.AL_models[source_id].decision_function(
                self.X_source[source_id][self.source_unlabeled_indices[source_id]]))
            return self.source_unlabeled_indices[source_id][np.argmin(uncertainty_list.reshape(-1))]
        elif criteria == "proximity":
            distance_list = mean_distance_source_instances_to_target(self.source_unlabeled_indices[source_id],
                                                                     self.X_target)
            return self.source_unlabeled_indices[source_id][np.argmin(distance_list.reshape(-1))]
        elif criteria == "KMM":
            return self.source_unlabeled_indices[source_id][
                np.argmax(self.alpha_list[source_id][self.source_unlabeled_indices[source_id]])]
        elif criteria == "KMM_Weighted_Uncertainty":
            alpha_list = self.max_alpha - self.alpha_list[source_id][self.source_unlabeled_indices[source_id]].reshape(
                -1)
